...
function answerCallback(string $callbackQueryId, string $text = '', bool $showAlert = false): bool {
  ...
  if (!($result['ok'] ?? false)) {
    ...
    if (strpos($errorMsg, 'query is too old') !== false || 
      ...
      // Callback expirou (mais de 3 horas)
      return false;
    }
  }
...
// callback velho
if (isset($update['callback_query']['message']['date'])) {
    if (($now - (int)$update['callback_query']['message']['date']) > 15) {
        echo 'ok'; exit;
    }
}
// ===== FIM =====
...
define('CMD_WINDOW_SECONDS', 600); // 10 minutos
...
// ignora callback velho (evita query is too old)
    if (isset($cb['message']['date']) && is_numeric($cb['message']['date'])) {
    if ((time() - (int)$cb['message']['date']) > 15) {
        echo 'ok'; exit;
    }
}
...
case 'VIP_BUY': {
    ...
    if ($ownerId > 0 && $fromId !== $ownerId) {
        tg('answerCallbackQuery', [
            'callback_query_id' => $cb['id'],
            'text' => '⛔ Apenas quem solicitou pode usar este botão.',
            'show_alert' => true
        ]);
        echo 'ok'; exit;
    }
...
case 'VIP_renovar': {
    ...
    tg('answerCallbackQuery', [
        'callback_query_id' => $cb['id'],
        'text' => '🔄 Abrindo opções de renovação...',
        'show_alert' => false
    ]);
...
case 'VIP_cancelar_voltar': {
    ...
    echo 'ok'; exit;
        ...
        case 'CONSULTA_CPF':      sendHowTo($chatId,$messageId,$fromId,$fromName,'cpf',$ownerId?:$fromId);      tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_CNPJ':     sendHowTo($chatId,$messageId,$fromId,$fromName,'cnpj',$ownerId?:$fromId);     tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_CEP':      sendHowTo($chatId,$messageId,$fromId,$fromName,'cep',$ownerId?:$fromId);      tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_NOME':     sendHowTo($chatId,$messageId,$fromId,$fromName,'nome',$ownerId?:$fromId);     tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_TELEFONE': sendHowTo($chatId,$messageId,$fromId,$fromName,'telefone',$ownerId?:$fromId); tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_PLACA':    sendHowTo($chatId,$messageId,$fromId,$fromName,'placa',$ownerId?:$fromId);    tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_IP':       sendHowTo($chatId,$messageId,$fromId,$fromName,'ip',$ownerId?:$fromId);       tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;
        case 'CONSULTA_BIN':      sendHowTo($chatId,$messageId,$fromId,$fromName,'bin',$ownerId?:$fromId);      tg('answerCallbackQuery',['callback_query_id'=>$cb['id']??'']); echo 'ok'; exit;

        case 'GER_GRUPOS': {
          if ($chatType !== 'private') {
            tg('answerCallbackQuery', [
              'callback_query_id'=>$cb['id']??'',
              'text'=>'⚠️ Use este botão apenas no privado com o bot.',
              'show_alert'=>true
            ]);
            echo 'ok'; exit;
          }
...
";
        ...
        default:
          tg('answerCallbackQuery', ['callback_query_id'=>$cb['id']??'','text'=>'⚠️ Ação inválida.','show_alert'=>false]);
          echo 'ok'; exit;
      }
    }
